    ...
    def _archive_system_prompt(language: str) -> str:
        if language == "english":
            return (
                ...
                "titles are DEFAULT GARBAGE from the recording software and DO NOT "
                "describe the content at all.\n"
                "CRITICAL RULES:\n"
                "- The current title (e.g. 'Furniture', 'Apexsf.kjh', 'Gh', 'Ggg') is "
                "meaningless - NEVER base your title, description, or tags on its words, "
                "and NEVER treat it as the video's subject. The video is gameplay, not "
                "whatever the old title's words say.\n"
                "- You cannot see the video and do NOT know which game or subject it "
                "shows, so NEVER name a specific game, person, event, or claim specific "
                "content. Use neutral archive phrasing ('old clip', 'archived gameplay').\n"
                "- Never use clickbait or false claims. Titles: short, 4-8 words. "
                "Descriptions: 1-3 short casual lines plus 2-3 hashtags mixing English "
                "and Arabic. Return one JSON object only."
            )
        return (
            ...
            "- You cannot see the video and do NOT know which game or subject it "
            "shows, so NEVER name a specific game, person, event, or claim specific "
            "content. Use neutral archive phrasing (e.g. 'مقطع قديم من الأرشيف').\n"
            "- Never use clickbait or false claims. Titles: short, 4-8 words. "
